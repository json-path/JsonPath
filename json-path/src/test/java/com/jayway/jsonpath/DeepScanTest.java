...
public class DeepScanTest extends BaseTest {
    ...
    public void when_deep_scanning_leaf_multi_props_work() {
        ...
        result = using(conf).parse("[{\"a\": \"a-val\", \"b\": \"b-val\", \"c\": \"c-val\"}, [1, 5], {\"a\": \"a-val\"}]").read("$..['a', 'c']");
    ...
    private void executeScanPath(Option... options) {
        ...
        List<Map<String, Object>> result = JsonPath.using(configuration).parse(json).read("$..array[0]");
