...
public class ArrayPathTokenTest extends TestBase {
    ...
    public void array_can_select_multiple_indexes() {
        ...
        assertThat(result).containsOnly(singletonMap("foo", "foo-val-0"), singletonMap("foo", "foo-val-1"));
    }
    ...
    public void array_can_be_sliced_from_2() {
        List<Map> result = read(ARRAY, "$[5:]");
        assertThat(result).containsOnly(singletonMap("foo", "foo-val-5"), singletonMap("foo", "foo-val-6"));
    }
    ...
    public void array_can_be_sliced_between() {
        ...
        assertThat(result).containsOnly(singletonMap("foo", "foo-val-2"), singletonMap("foo", "foo-val-3"));
    }
}
