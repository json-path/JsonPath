...
public class JacksonTest extends BaseTest {
    ...
    public void an_object_can_be_mapped_to_pojo() {
        ...
        fooBarBaz = JsonPath.using(JACKSON_CONFIGURATION).parseUtf8(json.getBytes(UTF_8)).read("$", FooBarBaz.class);
        assertThat(fooBarBaz.foo).isEqualTo("foo");
        assertThat(fooBarBaz.bar).isEqualTo(10L);
        assertThat(fooBarBaz.baz).isEqualTo(true);
    }
    ...
    @Test
    public // https://github.com/jayway/JsonPath/issues/275
    void single_quotes_work_with_in_filter() {
        ...
        final Object readFromSingleQuoteBytes = JsonPath.using(JACKSON_CONFIGURATION).parseUtf8(jsonArray.getBytes(UTF_8)).read("$.[?(@.foo in ['bar'])].foo");
        final Object readFromDoubleQuoteBytes = JsonPath.using(JACKSON_CONFIGURATION).parseUtf8(jsonArray.getBytes(UTF_8)).read("$.[?(@.foo in [\"bar\"])].foo");
