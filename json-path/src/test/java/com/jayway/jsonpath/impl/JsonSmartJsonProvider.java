...
public class JsonSmartJsonProvider extends AbstractJsonProvider {
    ...
    public JsonSmartJsonProvider() {
        this(JSONValue.defaultReader.DEFAULT_ORDERED);
    }

    public JsonSmartJsonProvider(JsonReaderI<?> mapper){
        this.mapper = mapper;
    }
    ...
    public Object parse(String json, boolean strict) {
        try {
            return createParser(strict).parse(json, mapper);
    ...
    public Object parse(InputStream jsonStream, String charset, boolean strict) throws InvalidJsonException {
        try {
            return createParser(strict).parse(new InputStreamReader(jsonStream, charset), mapper);
        } catch (ParseException e) {
            throw new InvalidJsonException(e);
    ...
    private JSONParser createParser(boolean strict) {
        return new JSONParser(strict?JSONParser.MODE_STRICTEST:JSONParser.MODE_PERMISSIVE);
    }
}
