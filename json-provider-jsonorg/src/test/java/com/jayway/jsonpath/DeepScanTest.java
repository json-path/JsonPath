...
public class DeepScanTest extends BaseTest {
    ...
    public void require_single_property_ok() {
        ...
        Configuration configuration = CONFIGURATION.addOptions(Option.REQUIRE_PROPERTIES);
    ...
    public void require_multi_property_all_match() {
        ...
        assertThat(result).isEqualTo(new ArrayList() {{ this.add(ab);this.add(ab);}});
    }
    ...
    public void require_multi_property_some_match() {
        ...
        assertThat(result).isEqualTo(new ArrayList() {{ this.add(ab);}});
    }
    ...
    public void scan_for_single_property() {
        ...
        assertThat(parse(json).read("$..['a']", List.class)).isEqualTo(
        		new ArrayList(){{ this.add("aa"); this.add(a); this.add("aa");}});
    }
    ...
    public void scan_for_property_path_missing_required_property() {
        ...
        assertThat(using(CONFIGURATION.addOptions(Option.REQUIRE_PROPERTIES)).parse(json).read("$..['a'].x", List.class)).containsExactly("xx", "xx");
    }
    ...
    public void scans_can_be_filtered() {
        ...
        assertThat(using(CONFIGURATION.addOptions(Option.REQUIRE_PROPERTIES)).parse(animals).read("$..[?(@.mammal == true)].color", List.class)
        		).isEqualTo(new ArrayList() {{this.add(brown);this.add(white);}});
    }
    ...
    private void executeScanPath(Option... options) {
        ...
        List<Map<String, Object>> result = JsonPath
                ...
                .parse(json)
                .read("$..array[0]");
        assertThat(result).isEqualTo(new ArrayList() {{ this.add(expected); }});
    }
}
