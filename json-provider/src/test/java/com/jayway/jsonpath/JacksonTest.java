...
public class JacksonTest extends BaseTestConfiguration {
    ...
    public void an_object_can_be_mapped_to_pojo() {
        ...
        FooBarBaz fooBarBaz = JsonPath.using(JACKSON_CONFIGURATION).parse(json, false).read("$", FooBarBaz.class);
    ...
    public void jackson_converts_dates() {
        ...
        Date date = JsonPath.using(JACKSON_CONFIGURATION).parse(json, false).read("$['date_as_long']", Date.class);
    ...
    public void single_quotes_work_with_in_filter() {
        ...
        final Object readFromSingleQuote = JsonPath.using(JACKSON_CONFIGURATION).parse(jsonArray, false).read("$.[?(@.foo in ['bar'])].foo");
        final Object readFromDoubleQuote = JsonPath.using(JACKSON_CONFIGURATION).parse(jsonArray, false).read("$.[?(@.foo in [\"bar\"])].foo");
