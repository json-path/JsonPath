...
public class TapestryJsonProvider extends AbstractJsonProvider {
  ...
  @Override
  public Object parse(final String json, boolean strict) throws InvalidJsonException {
    return new JSONObject(json);
  }

  @Override
  public Object parse(final InputStream jsonStream, final String charset, boolean strict) throws InvalidJsonException {
    Scanner sc = null;
    try {
      sc = new Scanner(jsonStream, charset);
      return parse(sc.useDelimiter("\\A").next(), strict);
    } finally {
      if (sc != null) {
        sc.close();
      }
