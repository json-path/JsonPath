...
public class PredicatePathTokenTest {
    ...
    public void a_filter_predicate_can_be_evaluated_on_string_criteria() {
        ...
        List result = JsonPath.read (ARRAY, "$[?(@.foo == 'foo-val-1')]");

        assertThat(result).isEqualTo(new ArrayList() {{ this.add(singletonMap("foo", "foo-val-1"));}});
    }

    @Test
    public void a_filter_predicate_can_be_evaluated_on_int_criteria() {

        List result = JsonPath.read (ARRAY2, "$[?(@.int == 1)]");

        assertThat(result).hasSize(1);
        assertThat(result).isEqualTo(new ArrayList() {{this.add(
        		new HashMap() {{
        			this.put("int",1);
        			this.put("decimal",0.1);
    ...
    public void a_filter_predicate_can_be_evaluated_on_decimal_criteria() {
        ...
        List result = JsonPath.read (ARRAY2, "$[?(@.decimal == 0.1)]");
    ...
    public void field_existence_can_be_checked() {
        ...
        assertThat(result).isEqualTo(new ArrayList() {{this.add(
        		new HashMap() {{
        			this.put("int",7);
        			this.put("decimal",0.7);
        			this.put("foo", "foo-val-7");
        			this.put("bool", true);
        		}});
        }});
    }
